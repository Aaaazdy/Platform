...
public class CProgressButton extends androidx.appcompat.widget.AppCompatButton implements ProgressListener
{
    ...
    private static String[] statusString = new String[]{"generate", "pause", "complete", "error", "delete"};
    ...
    private void setState(STATE state, boolean anim)
    {
        ...
        if (getWidth() == 0 || morphingCircle || morphingNormal) return;
        this.mState = state;
        if (anim)
        {
    ...
    private void morph2Normal()
    {
        ...
        @SuppressLint("ObjectAnimatorBinding") ObjectAnimator cornerAnimation = ObjectAnimator.ofFloat(mBackground, "cornerRadius", mToCornerRadius, mFromCornerRadius);

        final int start = (mWidth - mHeight) / 2;
        ValueAnimator animator = ValueAnimator.ofInt(start, 0);
        animator.setDuration(mDuration).addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
        {
            @Override
    ...
    private void morph2Circle()
    {
        ...
        @SuppressLint("ObjectAnimatorBinding") ObjectAnimator cornerAnimation = ObjectAnimator.ofFloat(mBackground, "cornerRadius", mFromCornerRadius, mToCornerRadius);
